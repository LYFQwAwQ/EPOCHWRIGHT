...
公共领域契约。包含版本常量、标准地图图层、战斗输入、模式判别联合、渲染帧、事件、岗位/换岗、运输与炮兵展开状态、目标/车辆/下车评估、平台部件与武器检查、最终结果和 `BattleSimulation` 接口。当前 schema/rules 为 `stage-3.1/stage-3.7`，并保留 `stage-3` 下既有规则和 `content-2` 的显式迁移常量/类型；炮兵事实事件已包含逻辑弹丸 ID 和权威弹着。
...
拥有 `content-3` 默认时代、步兵、单平台车辆和自行火炮编组，以及成员、平台、部件、岗位、展开规则、传感器与 fire mode 武器模板；同时负责 `content-1/2` 显式迁移、内容深拷贝、引用/数值/能力门禁验证和规范哈希。当前允许单一 `direct/resolved|logical-projectile` mode；要求展开的成员武器、缺少展开规则的平台武器、间射和越过安全整数/最大飞行 tick 的弹道会明确拒绝。

### `src/sim/internal.ts`

模拟私有运行时状态，包括成员位置、平台/部件/乘员分配、换岗进度、平台武器弹药与计时、炮兵展开状态/计时/计数、完整逻辑弹丸效果快照、硬截止结算快照、运输关系/动作/受困与一次性损伤标记、小队、感知、接触、情报队列、网格/掩体占用、掩体/目标/车辆/下车选择黑板与目标状态。此文件不是公共 API。

新增字段通常还需要更新 `runtime.ts` 的初始化、`getStateHash`、结果或检查投影，以及测试。

### `src/sim/setup.ts`

负责迁移、严格验证和哈希完整 `BattleSetup`。旧输入先升级到 `stage-3/content-2`，再统一迁移为当前 `stage-3.1/stage-3.7/content-3`；新的当前输入缺失内容、模板引用、平台数组、运输关系或编制槽位时明确拒绝。出生、撤离、任务和增援路线按编组平台的 `foot/wheeled/tracked` 移动类型经过实际 A* 验证；平台验证还覆盖稳定 ID、单车限制、朝向、岗位引用、资格、有效驾驶员和逻辑弹丸地图算术边界。`hashBattleSetup` 覆盖内容规范哈希、平台 spawn、初始乘员分配、运输关系和全部静态规则输入。
...
无状态炮兵物理规则。负责整数水平距离与飞行 tick、固定整数抛物线位置、稳定 supercover 格枚举、地形/静态对象首碰和整数爆炸衰减；不拥有弹丸集合、随机、tick 或伤害目标。`simulation.ts` 保存效果快照并决定推进、弹着实体枚举与同时结算。
