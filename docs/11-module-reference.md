...
只负责把 seed、规模和演示模式等便利选项生成完整的当前版本 `BattleSetup`，并显式附带默认 `content-2` 内容、模板 ID、平台数组和运输关系数组。生成结果在返回前走标准验证器；模块不创建模拟，也不属于外部系统必须依赖的战斗输入契约。

网页演示显式配置三方关系，生成器无参默认仍为两方步枪编组；`vehicleGroupsPerFaction` 可以为观察场景生成一组一车的轮式/履带编组。性能档位也只改写演示生成选项，最终仍向 Worker 发送完整 setup。

### `src/demo/scenarios.ts`

定义只用于开发观察端的稳定场景目录，把三方同盟冲突、双边冲突、车辆遭遇战、单/多目标防守和增援波次转换为 `DemoBattleSetupOptions`。它可以组合公开模式、车辆和增援输入，但不创建模拟、不绕过 `validateBattleSetup`，也不属于正式游戏输入协议。
...
拥有 `content-2` 默认时代、步兵与单平台车辆编组、成员、轮式/履带平台、部件、驾驶/炮手/替补岗位、传感器和步枪模板，以及 `content-1` 显式迁移、内容深拷贝、引用/数值/支持能力验证和规范哈希。模板字典按命名空间与稳定 ID 哈希；时代显示名等纯观察字段不影响战斗哈希。当前平台模板允许已实现的地面移动、观察、岗位替换和平台武器，不能通过内容包提前启用装甲伤害、运输、逻辑飞行体或能力脚本。

### `src/sim/internal.ts`

模拟私有运行时状态，包括成员位置、平台/部件/乘员分配、换岗进度、平台武器弹药与计时、小队、感知、接触、情报队列、网格/掩体占用、掩体选择黑板与目标状态。此文件不是公共 API。

新增字段通常还需要更新 `runtime.ts` 的初始化、`getStateHash`、结果或检查投影，以及测试。

### `src/sim/setup.ts`

负责迁移、严格验证和哈希完整 `BattleSetup`。`stage-2`/`stage-2.1`/`stage-2.2` 输入会转换为等价 `stage-3/content-2` 输入，`stage-3/stage-3.0` 输入会显式升级到当前岗位规则；新的 `stage-3` 输入缺失内容、模板引用、平台数组、运输关系或编制槽位时明确拒绝。出生、撤离、任务和增援路线按编组平台的 `foot/wheeled/tracked` 移动类型经过实际 A* 验证；初始平台还会验证稳定 ID、单车限制、朝向、岗位引用、资格与有效驾驶员。`hashBattleSetup` 覆盖内容规范哈希、平台 spawn、初始乘员分配、运输关系和全部静态规则输入。

演示、未来城市/养成系统和持久化加载器都通过相同的 `BattleSetup` 边界接入，并在创建运行时状态前走同一个验证器。

### `src/sim/map.ts`

包含参数化随机高度、山地、开阔水体、湿地和静态对象生成，以及地图验证与哈希、网格索引、步行/轮式/履带成本投影、可通行查询、高度查询、视线和距离函数。轮式偏好铺装且不能通过湿地，履带以较低粗糙地表代价通过湿地，车辆均拒绝深水；移动类型的相邻高差阈值由寻路和实际步骤共同消费。生成器使用独立 seed 流和稳定候选排序，按全图格数分配可满足的精确配额，并保留两侧部署带与跨图主通道；输入在分配数组前限制为最多 `512 x 512` 总格数和 `4:1` 长宽比。`map-2` 使用嵌套 TypedArray 图层，索引为 `z * width + x`。
...
静态地图按 `foot/wheeled/tracked` 分别派生连通分量和 Pathfinder 缓存，AI 目标会投影到编组当前移动类型所在分量，避免把局部可走但对该平台不可达的区域当作巡逻目标。该缓存由已哈希地图完全派生，不进入权威状态。
...
| `ui/Inspector.tsx` | 选中编组的行动原因、士气、压制、伤情、平台摘要、接触、掩体评估和路线 |
