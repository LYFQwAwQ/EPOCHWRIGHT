...
公共领域契约。包含版本常量、标准地图图层、战斗输入、模式判别联合、渲染帧、事件、飞行/岗位/换岗、运输与炮兵展开/任务 inspection、目标/车辆/下车评估、平台部件与武器检查、最终结果和 `BattleSimulation` 接口。当前 schema/rules 为 `stage-4/stage-4.0`，并保留阶段 3 与旧内容的显式迁移常量/类型；飞行高度进入 render/inspection/result，炮兵事实事件包含任务变化、逻辑弹丸 ID 和权威弹着。
...
模拟私有运行时状态，包括成员位置、平台飞行状态/部件/乘员分配、换岗进度、平台武器弹药与计时、炮兵展开状态/计时/计数、接触目标域/飞行快照/来源/送达 tick、间射任务快照/瞄准/评估、完整逻辑弹丸效果快照、硬截止结算快照、运输关系/动作/受困与一次性损伤标记、小队、感知、情报队列、地面占用/空域预约/掩体占用、选择黑板与目标状态。此文件不是公共 API。

新增字段通常还需要更新 `runtime.ts` 的初始化、`getStateHash`、结果或检查投影，以及测试。

### `src/sim/setup.ts`

负责迁移、严格验证和哈希完整 `BattleSetup`。旧输入沿显式迁移链统一转换为当前 `stage-4/stage-4.0/content-4`；新的当前输入缺失内容、模板引用、平台数组、运输关系或编制槽位时明确拒绝。出生、撤离、任务和增援路线按 `foot/wheeled/tracked/hover` 实际验证；悬停平台还验证高度带、量化离地高度、有效飞行员和初始同带安全间隔。`hashBattleSetup` 覆盖内容规范哈希、平台 spawn/高度带、初始乘员分配、运输关系和全部静态规则输入。
...
无状态空域规则。负责从地表与离地高度派生量化飞行高度，以及用整数/`BigInt` 平方距离精确判断同高度带安全半径冲突；不同高度带不冲突。它不拥有预约集合、路径、AI 或 tick，`setup.ts` 和 `simulation.ts` 分别负责初始部署与移动预约的稳定调用顺序。
...
| `ui/Inspector.tsx` | 选中编组的行动原因、士气、压制、伤情、平台/运输摘要、接触、掩体评估和路线；选中平台的飞行高度、部件、展开与炮兵任务检查 |
...
- `src/sim/air.test.ts`：覆盖内容/setup 迁移与负例、精确安全半径、空地共享、同带移动预约、占领隔离、高度视线/投影/结果、逐 tick 哈希与冻结。
