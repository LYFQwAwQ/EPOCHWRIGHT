...
只负责把 seed、规模和演示模式等便利选项生成完整的当前版本 `BattleSetup`，并显式附带默认 `content-4` 内容、模板 ID、平台数组和运输关系数组。生成结果在返回前走标准验证器；模块不创建模拟，也不属于外部系统必须依赖的战斗输入契约。

网页演示显式配置三方关系，生成器无参默认仍为两方步枪编组；`vehicleGroupsPerFaction` 和 `airGroupsPerFaction` 可以为观察场景生成一组一平台的地面车辆或悬停侦察编组。性能档位也只改写演示生成选项，最终仍向 Worker 发送完整 setup。

### `src/demo/scenarios.ts`

定义只用于开发观察端的稳定场景目录，把三方同盟冲突、双边冲突、车辆遭遇战、自行火炮观察、低空侦察、合成兵种防守、单/多目标防守和增援波次转换为 `DemoBattleSetupOptions`。它可以组合公开模式、平台和增援输入，但不创建模拟、不绕过 `validateBattleSetup`，也不属于正式游戏输入协议。

### `src/sim/types.ts`

公共领域契约。包含版本常量、标准地图图层、战斗输入、模式判别联合、渲染帧、事件、飞行/岗位/换岗、运输与炮兵展开/任务 inspection、目标/车辆/下车评估、平台部件与武器检查、最终结果和 `BattleSimulation` 接口。当前 schema/rules 为 `stage-4/stage-4.1`，并保留阶段 3、`stage-4.0` 与旧内容的显式迁移常量/类型；飞行快照进入 render/result，高度动作与候选分项只进入本方 inspection，炮兵事实事件包含任务变化、逻辑弹丸 ID 和权威弹着。
...
拥有 `content-4` 默认时代、步兵、单平台车辆、自行火炮和无武装观察直升机编组，以及成员、平台、飞行规则、部件、岗位、展开规则、传感器与 fire mode 武器模板；同时负责 `content-1/2/3` 显式迁移、内容深拷贝、引用/数值/能力门禁验证和规范哈希。悬停平台必须具备动力、升力、飞行员和合法安全半径/高度；地面平台不得携带飞行规则。

### `src/sim/internal.ts`

模拟私有运行时状态，包括成员位置、平台飞行快照/高度动作/高度评估、部件/乘员分配、换岗进度、平台武器弹药与计时、炮兵展开状态/计时/计数、接触目标域/飞行快照/来源/送达 tick、间射任务快照/瞄准/评估、完整逻辑弹丸效果快照、硬截止结算快照、运输关系/动作/受困与一次性损伤标记、小队、感知、情报队列、地面占用/空域预约/掩体占用、选择黑板与目标状态。此文件不是公共 API。

新增字段通常还需要更新 `runtime.ts` 的初始化、`getStateHash`、结果或检查投影，以及测试。

### `src/sim/setup.ts`

负责迁移、严格验证和哈希完整 `BattleSetup`。旧输入沿显式迁移链统一转换为当前 `stage-4/stage-4.1/content-4`；新的当前输入缺失内容、模板引用、平台数组、运输关系或编制槽位时明确拒绝。出生、撤离、任务和增援路线按 `foot/wheeled/tracked/hover` 实际验证；悬停平台还验证高度带、量化离地高度、有效飞行员和初始同带安全间隔。`hashBattleSetup` 覆盖内容规范哈希、平台 spawn/高度带、初始乘员分配、运输关系和全部静态规则输入。
...
无状态空域规则。负责高度带顺序与通用传感器/暴露修正、跨带动作 tick 和量化插值、低空目标格净空、整数高度候选评分，以及从地表/离地高度派生飞行高度和用整数/`BigInt` 平方距离判断同带安全半径冲突。它不拥有预约集合、接触、AI 黑板或 tick；`setup.ts` 和 `simulation.ts` 分别负责初始部署、动作推进与预约的稳定调用顺序。
...
拥有规范 `BattleSetup` 的深拷贝，以及成员、编组、平台位置/飞行/部件/岗位/武器/展开/运输初态、空逻辑弹丸集合、地面占用与空域预约索引、势力知识、目标和增援运行时状态的确定性初始化。悬停平台不进入地面占用；它不推进 tick，初始化后的权威状态仍由 `simulation.ts` 独占和调度。
...
- 只消费地形、公开任务和合法接触快照的高度效用、跨带容量、动作推进/中断，以及高度驱动的感知范围和目标暴露；
...
| `ui/Inspector.tsx` | 选中编组的行动原因、士气、压制、伤情、平台/运输摘要、接触、掩体评估和路线；选中平台的飞行高度/动作/原因、部件、展开与炮兵任务检查 |
...
- `tests/e2e/battle.spec.ts`：真实 Worker、WebGL、车辆/炮兵/悬停空军、观察权限、控制、模式、特征像素与响应式布局。
