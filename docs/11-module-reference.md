...
公共领域契约。包含版本常量、标准地图图层、战斗输入、模式判别联合、渲染帧、事件、岗位/换岗、运输状态、平台部件与武器检查、最终结果和 `BattleSimulation` 接口。当前规则为 `stage-3.3`，并保留 `stage-3.0`、`stage-3.1`、`stage-3.2` 的显式迁移版本常量。
...
模拟私有运行时状态，包括成员位置、平台/部件/乘员分配、换岗进度、平台武器弹药与计时、运输关系/动作/受困与一次性损伤标记、小队、感知、接触、情报队列、网格/掩体占用、掩体选择黑板与目标状态。此文件不是公共 API。

新增字段通常还需要更新 `runtime.ts` 的初始化、`getStateHash`、结果或检查投影，以及测试。

### `src/sim/setup.ts`

负责迁移、严格验证和哈希完整 `BattleSetup`。`stage-2`/`stage-2.1`/`stage-2.2` 输入会转换为等价 `stage-3/content-2` 输入，`stage-3/stage-3.0`、`stage-3.1`、`stage-3.2` 输入会显式升级到当前规则；新的 `stage-3` 输入缺失内容、模板引用、平台数组、运输关系或编制槽位时明确拒绝。出生、撤离、任务和增援路线按编组平台的 `foot/wheeled/tracked` 移动类型经过实际 A* 验证；初始平台还会验证稳定 ID、单车限制、朝向、岗位引用、资格与有效驾驶员，运输关系验证唯一乘客、同势力、容量、初始同锚点和同部署批次。`hashBattleSetup` 覆盖内容规范哈希、平台 spawn、初始乘员分配、运输关系和全部静态规则输入。
