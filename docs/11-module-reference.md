...
公共领域契约。包含版本常量、标准地图图层、战斗输入、模式判别联合、渲染帧、事件、岗位/换岗、运输状态、目标/车辆/下车评估、平台部件与武器检查、最终结果和 `BattleSimulation` 接口。当前规则为 `stage-3.5`，并保留 `stage-3.0`、`stage-3.1`、`stage-3.2`、`stage-3.3`、`stage-3.4` 的显式迁移版本常量。
...
拥有 `content-2` 默认时代、步兵与单平台车辆编组、成员、轮式/履带平台、部件、驾驶/炮手/替补岗位、运输容量/时间、传感器和步枪模板，以及 `content-1` 显式迁移、内容深拷贝、引用/数值/支持能力验证和规范哈希。模板字典按命名空间与稳定 ID 哈希；时代显示名等纯观察字段不影响战斗哈希。当前平台模板允许已实现的地面移动、观察、岗位替换、平台武器和显式配对运输，不能通过内容包提前启用逻辑飞行体或能力脚本。

### `src/sim/internal.ts`

模拟私有运行时状态，包括成员位置、平台/部件/乘员分配、换岗进度、平台武器弹药与计时、运输关系/动作/受困与一次性损伤标记、小队、感知、接触、情报队列、网格/掩体占用、掩体/目标/车辆/下车选择黑板与目标状态。此文件不是公共 API。

新增字段通常还需要更新 `runtime.ts` 的初始化、`getStateHash`、结果或检查投影，以及测试。

### `src/sim/setup.ts`

负责迁移、严格验证和哈希完整 `BattleSetup`。`stage-2`/`stage-2.1`/`stage-2.2` 输入会转换为等价 `stage-3/content-2` 输入，`stage-3/stage-3.0`、`stage-3.1`、`stage-3.2`、`stage-3.3` 输入会显式升级到当前规则；新的 `stage-3` 输入缺失内容、模板引用、平台数组、运输关系或编制槽位时明确拒绝。出生、撤离、任务和增援路线按编组平台的 `foot/wheeled/tracked` 移动类型经过实际 A* 验证；初始平台还会验证稳定 ID、单车限制、朝向、岗位引用、资格与有效驾驶员，运输关系验证唯一乘客、同势力、容量、初始同锚点和同部署批次。`hashBattleSetup` 覆盖内容规范哈希、平台 spawn、初始乘员分配、运输关系和全部静态规则输入。
...
拥有规范 `BattleSetup` 的深拷贝，以及成员、编组、平台位置/部件/岗位/武器/运输初态、占用索引、势力知识、目标和增援运行时状态的确定性初始化。平台初始能力由 `vehicle.ts` 派生，初始搭载由 `transport.ts` 原子建立并排除乘客地面占用。它不推进 tick；初始化后的权威状态仍由 `simulation.ts` 独占和调度。
