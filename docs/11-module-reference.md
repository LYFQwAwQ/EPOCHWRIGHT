...
只负责把 seed、规模和演示模式等便利选项生成完整的当前版本 `BattleSetup`，并显式附带默认 `content-9` 内容、模板 ID、平台数组和运输关系数组。生成结果在返回前走标准验证器；模块不创建模拟，也不属于外部系统必须依赖的战斗输入契约。

网页演示显式配置三方关系，生成器无参默认仍为两方步枪编组；`vehicleGroupsPerFaction` 可以生成一组一平台的地面车辆，`airGroupsPerFaction` 默认保持既有观察直升机语义，显式 `airGroupTypes` 才选择观察直升机、武装直升机或侦察无人机，`passiveAbilityGroupsPerFaction`、`auraAbilityGroupsPerFaction` 与 `activeAbilityGroupsPerFaction` 分别生成被动、持续光环和主动能力来源编组。性能档位也只改写演示生成选项，最终仍向 Worker 发送完整 setup。

### `src/demo/scenarios.ts`

定义只用于开发观察端的稳定场景目录，把三方同盟冲突、双边冲突、被动能力对抗、持续光环对抗、主动能力对抗、车辆遭遇战、自行火炮观察、低空侦察、空中行动、合成兵种防守、单/多目标防守和增援波次转换为 `DemoBattleSetupOptions`。它可以组合公开模式、平台、能力内容和增援输入，但不创建模拟、不绕过 `validateBattleSetup`，也不属于正式游戏输入协议。

### `src/sim/types.ts`

公共领域契约。包含版本常量、标准地图图层、战斗输入、模式判别联合、被动/光环/主动能力条件、目标和效果、渲染帧、事件、飞行/岗位/换岗、运输与炮兵展开/任务 inspection、目标/车辆/下车评估、平台部件与武器检查、最终结果和 `BattleSimulation` 接口。当前 schema/rules/content 为 `stage-4/stage-4.2/content-9`，并保留阶段 3、`stage-4.0`、`stage-4.1` 与旧内容的显式迁移类型；能力明细只进入本方/全知 inspection，临时光环不进入 result，主动能力使用摘要和飞行事实进入 result。
...
拥有 `content-9` 默认时代、步兵、被动/持续光环/主动能力步兵、单平台车辆、自行火炮、观察/武装直升机和侦察无人机编组，以及成员、能力、平台、飞行规则、部件、岗位、展开规则、传感器和武器模板；同时负责 `content-1..8` 显式迁移、内容深拷贝、引用/数值/能力门禁验证和规范哈希。能力显示名不进战斗哈希，条件、触发、目标、效果、范围、叠加、冷却、次数和成员引用全部进入。

### `src/sim/ability.ts`

提供无状态被动、光环和主动能力解析规则。被动按稳定成员 ID 和能力 ID 计算自身属性、所属编组聚合修正；光环按同势力整数网格范围生成稳定来源/应用候选，执行 `stack|strongest` 选择并保存应用开始 tick；主动能力建立来源状态、验证来源条件、按友军位置/在场/压制计算候选与稳定效用，并投影可解释检查。模块只接受已验证内容与成员/编组状态切片，不拥有或推进模拟 tick；`simulation.ts` 决定固定调用时机并结算次数、冷却、效果和事件。

### `src/sim/internal.ts`

模拟私有运行时状态，包括成员位置、成员来源的主动能力次数/冷却/使用序号/最近评估、活动光环应用、平台 `airborne|forced-landed|crashed` 飞行状态/高度动作/高度评估、部件/乘员分配、换岗进度、平台武器弹药与计时、炮兵展开状态/计时/计数、接触目标域/飞行快照/来源/送达 tick、间射任务快照/瞄准/评估、完整逻辑弹丸效果快照、硬截止结算快照、运输关系/动作/受困与一次性损伤标记、小队、感知、情报队列、地面占用/空域预约/掩体占用、选择黑板与目标状态。此文件不是公共 API。

新增字段通常还需要更新 `runtime.ts` 的初始化、`getStateHash`、结果或检查投影，以及测试。

### `src/sim/setup.ts`

负责迁移、严格验证和哈希完整 `BattleSetup`。旧输入沿显式迁移链统一转换为当前 `stage-4/stage-4.2/content-9`；新的当前输入缺失内容、模板引用、平台数组、运输关系或编制槽位时明确拒绝。出生、撤离、任务和增援路线按 `foot/wheeled/tracked/hover` 实际验证；`hashBattleSetup` 覆盖能力内容规范哈希、平台 spawn/高度带、初始乘员分配、运输关系和全部静态规则输入。
...
拥有规范 `BattleSetup` 的深拷贝，以及成员、成员主动能力、编组、初始活动光环、平台位置/飞行/部件/岗位/武器/展开/运输初态、空逻辑弹丸集合、地面占用与空域预约索引、势力知识、目标和增援运行时状态的确定性初始化。悬停平台以 `airborne` 初始化且不进入地面占用；它不推进 tick，初始化后的权威状态仍由 `simulation.ts` 独占和调度。
...
| `render/Units.tsx` | 成员实例、轮式/履带/自行火炮平台、带旋翼动画的悬停直升机、编组标记、实体选择反馈和位置插值 |
...
| `ui/Inspector.tsx` | 选中编组的行动原因、被动/持续光环、主动能力次数/冷却/候选原因、士气、压制、伤情、平台/运输摘要、接触、掩体评估和路线；选中平台的飞行高度/动作/原因、部件、展开与炮兵任务检查 |
