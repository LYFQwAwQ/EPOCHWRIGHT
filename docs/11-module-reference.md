...
只负责把 seed、规模和演示模式等便利选项生成完整的当前版本 `BattleSetup`，并显式附带默认 `content-3` 内容、模板 ID、平台数组和运输关系数组。生成结果在返回前走标准验证器；模块不创建模拟，也不属于外部系统必须依赖的战斗输入契约。
...
定义只用于开发观察端的稳定场景目录，把三方同盟冲突、双边冲突、车辆遭遇战、合成兵种防守、单/多目标防守和增援波次转换为 `DemoBattleSetupOptions`。它可以组合公开模式、车辆和增援输入，但不创建模拟、不绕过 `validateBattleSetup`，也不属于正式游戏输入协议。

### `src/sim/types.ts`

公共领域契约。包含版本常量、标准地图图层、战斗输入、模式判别联合、渲染帧、事件、岗位/换岗、运输与炮兵展开/任务 inspection、目标/车辆/下车评估、平台部件与武器检查、最终结果和 `BattleSimulation` 接口。当前 schema/rules 为 `stage-3.1/stage-3.8`，并保留 `stage-3` 下既有规则和 `content-2` 的显式迁移常量/类型；炮兵事实事件包含任务变化、逻辑弹丸 ID 和权威弹着。
...
拥有 `content-3` 默认时代、步兵、单平台车辆和自行火炮编组，以及成员、平台、部件、岗位、展开规则、传感器与 fire mode 武器模板；同时负责 `content-1/2` 显式迁移、内容深拷贝、引用/数值/能力门禁验证和规范哈希。当前支持平台 `direct/resolved|logical-projectile` 与 `indirect/logical-projectile` mode；成员间射、要求展开但缺少展开规则的平台武器、非法误差参数和越过安全整数/最大飞行 tick 的弹道会明确拒绝。

### `src/sim/internal.ts`

模拟私有运行时状态，包括成员位置、平台/部件/乘员分配、换岗进度、平台武器弹药与计时、炮兵展开状态/计时/计数、接触来源/送达 tick、间射任务快照/瞄准/评估、完整逻辑弹丸效果快照、硬截止结算快照、运输关系/动作/受困与一次性损伤标记、小队、感知、情报队列、网格/掩体占用、掩体/目标/车辆/下车选择黑板与目标状态。此文件不是公共 API。

新增字段通常还需要更新 `runtime.ts` 的初始化、`getStateHash`、结果或检查投影，以及测试。

### `src/sim/setup.ts`

负责迁移、严格验证和哈希完整 `BattleSetup`。旧输入先升级到 `stage-3/content-2`，再统一迁移为当前 `stage-3.1/stage-3.8/content-3`；新的当前输入缺失内容、模板引用、平台数组、运输关系或编制槽位时明确拒绝。出生、撤离、任务和增援路线按编组平台的 `foot/wheeled/tracked` 移动类型经过实际 A* 验证；平台验证还覆盖稳定 ID、单车限制、朝向、岗位引用、资格、有效驾驶员和逻辑弹丸地图算术边界。`hashBattleSetup` 覆盖内容规范哈希、平台 spawn、初始乘员分配、运输关系和全部静态规则输入。
...
拥有规范 `BattleSetup` 的深拷贝，以及成员、编组、平台位置/部件/岗位/武器/展开/运输初态、空逻辑弹丸集合、占用索引、势力知识、目标和增援运行时状态的确定性初始化。具有展开规则的平台从 `packed` 和零计数开始；它不推进 tick，初始化后的权威状态仍由 `simulation.ts` 独占和调度。
...
静态地图按 `foot/wheeled/tracked` 分别派生连通分量和 Pathfinder 缓存，AI 目标会投影到编组当前移动类型所在分量，避免把局部可走但对该平台不可达的区域当作巡逻目标。最后已知接触落在弃置或摧毁平台的静态占用格时，目标分配会确定性选择最近可达格，避免进入空路径停滞。该缓存由已哈希地图完全派生，不进入权威状态。
