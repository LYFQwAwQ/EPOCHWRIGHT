...
`ARTILLERY-002` 已落地 `artillery-deploy-001`：覆盖 20 tick 展开、16 tick 收炮、未收炮移动阻断、未展开开火阻断、岗位失效取消、逐 tick 双实例哈希、平台结果/零弹丸 settlement 和终止冻结。内容/迁移用例逐字段断言 `content-2` 武器变为单一 `direct` mode，并拒绝错误岗位引用、成员展开武器、逻辑弹丸和间射提前启用。零速度、超长飞行和弹丸终止边界由 `ARTILLERY-003` 补齐。
