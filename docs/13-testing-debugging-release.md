...
`AIR-001` 的固定高度悬停回归位于 `src/sim/air.test.ts`：覆盖 `stage-3.1/stage-3.8/content-3` 迁移、飞行内容/setup 负例、整数安全半径边界、空地同格与不同高度带共存、同带同时移动的稳定预约、地面占用/目标占领隔离、目标域拒绝、权威高度视线、render/inspection/result 投影、逐 tick 双实例哈希和终止冻结。渲染纯函数另验证直升机本地机头轴经模型偏移后对齐模拟前向；`air-recon` 的真实 Worker/WebGL E2E 断言双方飞行平台、低空 `12m` 检查、选中模型特征像素、首次实际位移与权威航向同向、Canvas 非空，以及桌面和 `390 x 844` 视口无运行错误。
