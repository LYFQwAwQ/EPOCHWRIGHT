...
车辆伤害的固定回归位于 `src/sim/vehicle.test.ts`。纯规则用例固定装甲面、穿透边界、加权部件与完整度状态；场景用例固定未穿透外露损伤、部件/乘员独立结算、弃车和终止冻结，并使用 `simultaneous-438217` 验证两辆平台在同一 tick 先完整生成射击意图，再同时失去武器部件。
