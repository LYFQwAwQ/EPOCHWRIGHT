...
`ARTILLERY-002/003` 已落地 `artillery-deploy-001`、`artillery-direct-002`、`artillery-impact-005` 和 `artillery-finish-006`：覆盖 20 tick 展开、16 tick 收炮、未收炮移动阻断、未展开开火阻断、岗位失效取消、整数飞行/静态首碰、发射 tick 不命中、发射方失效后继续飞行、敌对爆区与非敌对免疫、同 tick 直射先收集、逐 tick 双实例哈希、硬截止 settling、结果无残留和终止冻结。内容/迁移用例逐字段断言 `content-2` 武器变为单一 `direct` mode、`stage-3.6` 显式升级，并拒绝错误岗位引用、成员展开武器、零速度/超界弹道和间射提前启用。

`ARTILLERY-005` 增加真实 Worker/WebGL E2E：固定自行火炮场景中可观察展开状态、至少一颗非空移动弹丸和弹着效果；全知/势力视角裁剪目标与预定弹着信息；暂停、单步、质量切换和窄屏布局保持既有约束。Canvas 检查必须同时验证弹丸连续两个逻辑帧的位置变化和弹着特征像素，不能只检查事件文本。
