...
`flightRule.clearanceMmByBand` 至少定义一个高度带；每个高度必须为正整数，并在具体 setup 验证时对齐地图高度量化。运行时保存当前 `{ state, altitudeBand, clearanceMm }`，其中 `state` 为 `airborne|forced-landed|crashed`；`AIR-002` 另保存目标带、源/目标离地高度、总/剩余 tick 和高度候选评估，不改写 spawn 含义。`AIR-003` 的攻击候选分项和最终飞行状态进入哈希与本方 inspection。安全半径必须为正整数，只有 `airborne` 平台参与空中移动、部署和跨带容量，并使用 `02 §5` 的成对距离规则。
...
interface ActiveAbilityTemplate {
  readonly id: TemplateId;
  readonly displayName: string;
  readonly tags: readonly string[];
  ...
  readonly conditions: readonly AbilityCondition[];
  readonly triggerConditions: readonly {
    readonly kind: "target-suppression";
    readonly minimumBps: number;
  }[];
  readonly effects: readonly SuppressionRecoveryAbilityEffect[];
}
...
interface SuppressionRecoveryAbilityEffect {
...
`content-8` 起的 `aura` 建立权威的活动来源快照。来源 ID 固定为长度前缀格式 `aura:<memberIdLength>:<sourceMemberId>:<abilityIdLength>:<abilityTemplateId>`，应用 ID 再以相同方式追加目标编组 ID，避免合法 ID 中的分隔符产生碰撞；初始化时生成一次，此后每个 tick 在移动完成后、武器结算前刷新。来源成员必须健康或受伤、仍为 `deployed`，并额外满足模板条件；因此本 tick 武器结算中失能或死亡、以及随后撤离的来源会在下一 tick 的同一刷新阶段移除，结算中途不改变已冻结的光环集合。
...
`content-9` 的 `active` 为每个来源成员和能力模板建立稳定状态 ID `active:<memberIdLength>:<sourceMemberId>:<abilityIdLength>:<abilityTemplateId>`，保存剩余次数、冷却截止 tick、使用序号、最近使用 tick 和最近一次候选评估。成功使用才消耗一次次数；在 tick `t` 使用且 `cooldownTicks = n` 时，下一次最早可在 `t + n` 评估为可用。当前注册效果 `suppression-recovery` 只改变同势力编组的权威压制，最终钳制到 `0..10_000`。

主动候选在移动与光环刷新后、武器结算前评估。`own-group` 只考虑来源编组；`nearby-friendly-groups` 考虑整数半径内仍有健康或受伤部署成员的同势力编组。`target-suppression` 使用目标当前权威压制，分数为实际可恢复压制；同分按目标编组 ID 升序选择。AI 不读取敌方实体、接触或观察投影，因此隐藏敌军真值变化不能改变主动能力选择。来源条件未满足、冷却中、次数耗尽、目标不可用、超距、阈值未满足或无收益都只保存 inspection 原因，不产生失败事件。

本方或全知 inspection 返回被动能力条件、当前命中目标编组的光环应用，以及来源编组主动能力的次数、冷却、候选分项、选择和拒绝原因；敌方已知接触不返回这些字段。`ability-used` 只记录成功事实。`BattleResult.groups[].activeAbilities` 保存稳定能力状态 ID、来源成员、模板 ID、使用次数、剩余次数和最近使用 tick；结束瞬间的临时光环集合仍不持久化。
...
type AirFlightState = "airborne" | "forced-landed" | "crashed";

interface PlatformFlightInspection {
...
`RenderPlatform.flight` 和接触快照只携带观察时的飞行状态、高度带与离地高度。`PlatformInspection.flightControl` 只向本方或全知检查公开动作、剩余 tick、选择原因和整数候选分项；它不得进入敌方最后已知快照，也不得由 UI 反向成为 AI 输入。迫降或坠毁后 `clearanceMm = 0`，不再提供高度控制动作。
...
主动能力成功时产生一次 `ability-used`，携带稳定使用 ID、来源成员/编组、能力模板、目标编组、从 1 开始的来源内使用序号、使用后剩余次数和本次实际恢复的压制。拒绝、冷却等待和次数耗尽不是瞬时事实，不发事件；它们保存在 inspection 的最近评估中。势力视角只接收本方来源的能力事件，客户端二次过滤还要求来源与目标都在当前可见编组集合中。
