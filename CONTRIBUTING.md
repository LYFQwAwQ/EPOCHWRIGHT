...
首次参与项目先读 [文档上下文与阅读指南](./docs/00-context-guide.md) 和 [当前实现状态](./docs/09-implementation-status.md)，然后按任务选择领域文档。不要顺序通读 `docs/01-13`：`01-08` 包含大量长期目标，范围清晰的改动只需读取相关章节。

自动化开发代理从 [AGENTS.md](./AGENTS.md) 开始，并遵循其中的任务路由；人类开发者可以从 [文档索引](./docs/README.md) 选择入口。

## 日常工作流

1. 用固定 seed 重现问题或建立最小场景。
2. 确认改动所属模块及边界契约。
3. 先补纯规则或场景测试，再实现行为。
4. 需要展示的新状态通过 `RenderFrame`、事件或按需检查结果公开。
5. 运行与风险相匹配的验证。
6. 更新实现状态或接口文档。
